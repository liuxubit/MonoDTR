...
class KittiMonoValDataset(torch.utils.data.Dataset):
    """Some Information about KittiDataset"""
    def __init__(self, cfg, split='val'):
        super(KittiMonoValDataset, self).__init__()
        ...
        self.imdb = []
        with open(os.path.join(cfg.path.val, 'val.txt')) as f:
            res = f.readlines()
            for i in range(len(res)):
                self.imdb.append(res[i].strip())
        ...
        self.kitti_data = KittiData(cfg.path.val, self.output_dict) 
    ...
    def _reproject(self, P2:np.ndarray, transformed_label:List[KittiObj]) -> Tuple[List[KittiObj], np.ndarray]:
        bbox3d_state = np.zeros([len(transformed_label.data), 7]) #[camera_x, camera_y, z, w, h, l, alpha]
        for obj in transformed_label.data:
            obj.alpha = theta2alpha_3d(obj.ry, obj.x, obj.z, P2)
        bbox3d_origin = torch.tensor([[obj.x, obj.y - 0.5 * obj.h, obj.z, obj.w, obj.h, obj.l, obj.alpha] for obj in transformed_label.data], dtype=torch.float32)
        abs_corner, homo_corner, _ = self.projector(bbox3d_origin, bbox3d_origin.new(P2))
        for i, obj in enumerate(transformed_label.data):
        ...
        if self.is_reproject:
            for i in range(len(transformed_label.data)):
                transformed_label.data[i].bbox_l = bbox2d[i, 0]
                transformed_label.data[i].bbox_t = bbox2d[i, 1]
                transformed_label.data[i].bbox_r = bbox2d[i, 2]
                transformed_label.data[i].bbox_b = bbox2d[i, 3]
        
        return transformed_label, bbox3d_state


    def __getitem__(self, index):
        print(f"index = {index} self.imdb[index] = {self.imdb[index]}")
        calib, image, label, depth = self.kitti_data.read_data(self.imdb[index])
        # print(f"\nlabel = {label}")
        # print(f"\nlen(label) = {len(label)}")
        ...
        # print(f"\ntransformed_label.data[0] = {transformed_label.data[0]}")
        print(f"\nlen(transformed_label.data) = {len(transformed_label.data)}")
        bbox3d_state = np.zeros([len(transformed_label.data), 7]) #[camera_x, camera_y, z, w, h, l, alpha]
        if len(transformed_label.data) > 0:
        ...
        bbox2d = np.array([[obj.bbox_l, obj.bbox_t, obj.bbox_r, obj.bbox_b] for obj in transformed_label.data])
        
        output_dict = {'calib': transformed_P2,
                       'image': transformed_image,
                       'label': [obj.type for obj in transformed_label.data], 
...
@DATASET_DICT.register_module
class KittiMonoTestDataset(KittiMonoValDataset):
...
if __name__ == '__main__':
    ...
    dataset = KittiMonoValDataset(cfg)
